...
const startRecognitionButton = document.getElementById('start-recognition');
        ...
        document.getElementById("start-summarizing").addEventListener("click", function() {
            var inputString = document.getElementById("input-text").value;
            
            // Make an AJAX request to the server
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/process_string", true);
            xhr.setRequestHeader("Content-Type", "application/json");
            xhr.onreadystatechange = function () {
                if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                    // Handle the response from the server
                    var response = JSON.parse(xhr.responseText);
                    var result = response[0]; // Accessing the first element of the tuple
                    var lengthSummarizedtext = response[1];
                    var lengthoriginaltext = response[2];
                    document.getElementById("output-text").textContent = result;
                }
            };
            xhr.send(JSON.stringify({ string: inputString }));
        });